...
public:
  ...
  inline double getDistance2d(const Eigen::Vector3i &id);
  ...
  void interpolateTrilinearEDT(double values[2][2][2],
                               const Eigen::Vector3d &diff, double &value);
  void interpolateTrilinearFirstGrad(double values[2][2][2],
                                     const Eigen::Vector3d &diff,
                                     Eigen::Vector3d &grad);

  void evaluateEDT(const Eigen::Vector3d &pos, double &dist);
  void evaluateEDT2d(const Eigen::Vector2d &pos, double &dist);
  void evaluateFirstGrad(const Eigen::Vector3d &pos, Eigen::Vector3d &grad);
  void evaluateFirstGrad2d(const Eigen::Vector2d &pos, Eigen::Vector2d &grad);
...
inline int GridMap::getInflateOccupancy2d(Eigen::Vector2d pos) {
  ...
  Eigen::Vector3i id;
  Eigen::Vector3d tem_pos(pos(0), pos(1), 0);
  posToIndex(tem_pos, id);
...
inline double GridMap::getDistance2d(const Eigen::Vector2d &pos) {
  Eigen::Vector3i id;
  Eigen::Vector3d tem_pos(pos(0), pos(1), 0);
  posToIndex(tem_pos, id);
  boundIndex(id);
...
inline double GridMap::getDistance2d(const Eigen::Vector3i &id) {
  Eigen::Vector3i id1 = id;
  boundIndex(id1);
  return md_.distance_buffer_all_[toAddress(id1)];
}
