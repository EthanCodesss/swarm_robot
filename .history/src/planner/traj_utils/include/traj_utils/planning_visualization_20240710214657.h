...
private:
  ...
  std::vector<Eigen::Vector2d> swarm_odom;
...
public:
  ...
  void displayMarkerList(ros::Publisher &pub,
                         const vector<Eigen::Vector2d> &list, double scale,
  ...
  void generateArrowDisplayArray(visualization_msgs::MarkerArray &array,
                                 const vector<Eigen::Vector2d> &list,
                                 double scale, Eigen::Vector4d color, int id);
  void displayGoalPoint(Eigen::Vector2d goal_point, Eigen::Vector4d color,
                        const double scale, int id);
  void displayGlobalPathList(vector<Eigen::Vector2d> global_pts,
                             const double scale, int id);
  void displayInitPathList(vector<Eigen::Vector2d> init_pts, const double scale,
                           int id);
  void displayMultiInitPathList(vector<vector<Eigen::Vector2d>> init_trajs,
                                const double scale);
  void displayOptimalList(Eigen::MatrixXd optimal_pts, int id);
  void displayFailedList(Eigen::MatrixXd failed_pts, int id);
  void displayAStarList(std::vector<std::vector<Eigen::Vector2d>> a_star_paths,
                        int id);
  void displayArrowList(ros::Publisher &pub,
                        const vector<Eigen::Vector2d> &list, double scale,
                        Eigen::Vector4d color, int id);
  void displayInitPathListDebug(vector<Eigen::Vector3d> init_pts,
                                const double scale, int id);
