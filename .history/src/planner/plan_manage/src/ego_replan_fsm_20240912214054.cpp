...
void SimuResults::write_header(std::ofstream& file) const {
    for (int i = 0; i < num_iters_; ++i) {
        file << "Cost Iter " << i << ",";
    }
    file << "Computation Time,";
    for (int i = 0; i < num_iters_; ++i) {
        file << "Computation Time Iter " << i << ",";
    }
    for (int i = 0; i < num_iters_; ++i) {
        file << "Velocity vx " << i << ",";
        file << "Velocity vy " << i << ",";

        file << "Acceleration ax " << i << ",";
        file << "Acceleration ay " << i << ",";
    ...
    for (int i = 0; i < num_neighbors_; ++i) {
        for (int c = 0; c < num_iters_; ++c) {
            file << "Inter rob " << i << "iter " << c << ",";
        }
    }
    file << std::endl;
}
void SimuResults::export_results(const std::string& path) const {
...
void SimuResults::write_data(std::ofstream& file) const {
    for (const auto& val : cost_iter_) {
        file << val << ",";
    }

    for (const auto& val : computation_time_iter_) {
        file << val << ",";
    }
    ...
    for (const auto& acc : accelaration_) {
        file << acc[0] << ",";
        file << acc[1] << ",";

    }
    for (const auto& cv : min_dis_obstacle_) {
        file << cv << ",";
    }
    for (const auto& iter : dis_robot_) {
...
void EGOReplanFSM::init(ros::NodeHandle &nh) {
  ...
  nh.param("fsm/result_file", result_fn_,
           string("/home/zuzu/Documents/Benchmark/21-RSS-ego-swarm/2.24/ego/"
                  "ego_swarm.txt"));
  ...
  broadcast_ploytraj_sub_ = nh.subscribe<traj_utils::PolyTraj>(
      ...
      &EGOReplanFSM::RecvBroadcastPolyTrajCallback, this,
  ...
  case EXEC_TRAJ: {
        ...
        0.1) // local target close to the global target
    {
      if (t_cur > info->duration - 0.2) {
        ...
        std::cout<<cost_iter_.size()<<std::endl;
        SimuResults results = SimuResults(cost_iter_, velocity_, accelaration_, dis_robot_, computation_time_iter_, min_dis_obstacle_);

        results.export_results("/home/ethan/swarm_robot/results/simu_results"+std::to_string(planner_manager_->pp_.drone_id)+".csv");
...
void EGOReplanFSM::checkCollisionCallback(const ros::TimerEvent &e) {
  ...
  for (double t = t_cur; t < info->duration; t += time_step) {
    // If t_cur < t_2_3, only the first 2/3 partition of the trajectory is
    // considered valid and will get checked.
    if (t_cur < t_2_3 && t >= t_2_3)
      break;

    if (map->getInflateOccupancy2d(info->traj.getPos(t)) == 1) {
...
void EGOReplanFSM::waypointCallback(const geometry_msgs::PoseStampedPtr &msg) {
  ...
  success = planner_manager_->planGlobalTrajWaypoints(
      odom_pos_, odom_vel_, Eigen::Vector2d::Zero(), one_pt_wps,
...
void EGOReplanFSM::RecvBroadcastPolyTrajCallback(
    const traj_utils::PolyTrajConstPtr &msg) {
  if (msg->drone_id < 0) {
  ...
  for (int i = 0; i < piece_nums; ++i) {
    int i6 = i * 6;
    cMats[i].row(0) << msg->coef_x[i6 + 0], msg->coef_x[i6 + 1],
        msg->coef_x[i6 + 2], msg->coef_x[i6 + 3], msg->coef_x[i6 + 4],
        msg->coef_x[i6 + 5];
...
void EGOReplanFSM::formationWaypointCallback(
  ...
  relative_pos << swarm_relative_pts_[id][0], swarm_relative_pts_[id][1];
...
void EGOReplanFSM::planGlobalTrajbyGivenWps() {
  ...
  bool success = planner_manager_->planGlobalTrajWaypoints(
      odom_pos_, Eigen::Vector2d::Zero(), Eigen::Vector2d::Zero(), wps,
      Eigen::Vector2d::Zero(), Eigen::Vector2d::Zero());
...
bool EGOReplanFSM::planFromLocalTraj(bool flag_use_poly_init,
  ...
  // cout << "planFromLocalTraj : " << ros::Time::now().toSec() - t_debug_start
  // << endl;
