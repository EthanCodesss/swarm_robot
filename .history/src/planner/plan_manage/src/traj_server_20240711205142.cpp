...
void finishCallback(const std_msgs::Bool::ConstPtr &msg) {
  if (msg->data == true) {
    ROS_WARN(
        "total_time, cnt, acc2_inter, jerk2_inter = %lf \t %d \t %lf \t %lf",
        (ros::Time::now() - global_start_time).toSec(), cnt, acc2_inter,
        jerk2_inter);
    // ROS_WARN("time.size = %d, %d, %d, %d", time_vec_.size(), vel_vec_.size(),
    // acc_vec_.size(), jerk_vec_.size());
    auto t =
...
std::pair<double, double> calculate_yaw(double t_cur, Eigen::Vector2d &vel,
  ...
  yaw_dot = (ddsigma.transpose() * B_h * dsigma)(0, 0) / dsigma.squaredNorm();
