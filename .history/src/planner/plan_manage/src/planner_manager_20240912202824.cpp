...
bool EGOPlannerManager::computeInitReferenceState(
    ...
    const Eigen::Vector2d &local_target_vel, const double &ts,
  ...
  if (flag_first_call || flag_polyInit) {
    ...
    // traj = ploy_traj_opt_->astarWithMinTraj(headState, tailState,
    // simple_path, ctl_points);
    ploy_traj_opt_->astarWithMinTraj(headState, tailState, simple_path,
                                     ctl_points, initMJO);
  ...
  /*** case 2: initialize from previous optimal trajectory ***/
    ...
    Eigen::Matrix<double, 2, 3> headState, tailState;
    Eigen::MatrixXd innerPs(2, piece_nums - 1);
    Eigen::VectorXd piece_dur_vec =
...
void EGOPlannerManager::getLocalTarget(const double planning_horizen,
                                       ...
                                       const Eigen::Vector2d &global_end_pt,
                                       Eigen::Vector2d &local_target_pos,
                                       Eigen::Vector2d &local_target_vel) {
  double t;
  ...
  if ((t - traj_.global_traj.global_start_time) >=
      traj_.global_traj.duration) // Last global point
  {
...
bool EGOPlannerManager::reboundReplan(
    const Eigen::Vector2d &start_pt, const Eigen::Vector2d &start_vel,
    const Eigen::Vector2d &start_acc, const double trajectory_start_time,
    const Eigen::Vector2d &local_target_pt,
    const Eigen::Vector2d &local_target_vel, const bool flag_polyInit,
    const bool flag_randomPolyTraj, const bool use_formation,
  ...
  if (!computeInitReferenceState(start_pt, start_vel, start_acc,
                                 local_target_pt, local_target_vel, ts, initMJO,
                                 flag_polyInit)) {
    return false;
  }

  Eigen::MatrixXd cstr_pts =
      initMJO.getInitConstrainPoints(ploy_traj_opt_->get_cps_num_prePiece_());
  ...
  flag_success = ploy_traj_opt_->OptimizeTrajectory_lbfgs(
      headState, tailState, innerPts, initTraj.getDurations(), cstr_pts,
      use_formation);
  ...
  cout << "total time:\033[42m" << (t_init + t_opt).toSec()
       << "\033[0m,init:" << t_init.toSec() << ",optimize:" << t_opt.toSec()
  ...
  if (have_local_traj && use_formation) {
    ...
    traj_.setLocalTraj(ploy_traj_opt_->getMinJerkOptPtr()->getTraj(),
                       trajectory_start_time);
  } else {
    traj_.setLocalTraj(ploy_traj_opt_->getMinJerkOptPtr()->getTraj(),
                       ros::Time::now().toSec()); // todo time
  }
...
bool EGOPlannerManager::planGlobalTrajWaypoints(
  ...
  if (waypoints.size() > 1) {
    innerPts.resize(2, waypoints.size() - 1);
