...
bool PolyTrajOptimizer::OptimizeTrajectory_lbfgs(
  ...
  Eigen::Vector2d start_pos = iniState.col(0);
...
bool PolyTrajOptimizer::checkCollision(void) {
  ...
  T_end = durations.head(piece_of_idx).sum() +
          durations(piece_of_idx) * (idx - piece_of_idx * cps_num_prePiece_) /
              cps_num_prePiece_;
  ...
  for (int i = 0; i < i_end; i++) {
    ...
    if (grid_map_->getInflateOccupancy2d(pos) == 1) {
      occ = true;
      break;
    }
    t += dt;
...
double PolyTrajOptimizer::costFunctionCallback(void *func_data, const double *x,
  ...
  // 1]))); // same t
  Eigen::Map<const Eigen::VectorXd> t(x + (2 * (opt->piece_num_ - 1)),
                                      opt->piece_num_);
  Eigen::Map<Eigen::MatrixXd> gradP(grad, 2, opt->piece_num_ - 1);
  Eigen::Map<Eigen::VectorXd> gradt(grad + (2 * (opt->piece_num_ - 1)),
                                    opt->piece_num_);
  ...
  opt->addPVAGradCost2CT(gradT, obs_swarm_feas_qvar_costs,
...
// 添加位置， 速度， 加速度的梯度成本到约束矩阵中， 并计算相应的成本值
template <typename EIGENVEC>
void PolyTrajOptimizer::addPVAGradCost2CT(EIGENVEC &gdT, Eigen::VectorXd &costs,
                                          const int &K) {

  int N = gdT.size();
  Eigen::Vector2d pos, vel, acc, jer;
  Eigen::Vector2d gradp, gradv, grada;
  ...
  for (int i = 0; i < N; ++i) {
    const Eigen::Matrix<double, 6, 2> &c =
        jerkOpt_.get_b().block<6, 2>(i * 6, 0);
    ...
    for (int j = 0; j <= K; ++j) {
      ...
      // formation
      if (use_formation_) {
        if (swarmGraphGradCostP(i_dp, t + step * j, pos, vel, gradp, gradt,
                                grad_prev_t, costp)) {
          gradViolaPc = beta0 * gradp.transpose();
          gradViolaPt = alpha * gradt;
          jerkOpt_.get_gdC().block<6, 2>(i * 6, 0) += omg * step * gradViolaPc;
...
bool PolyTrajOptimizer::swarmGraphGradCostP(const int i_dp, const double t,
                                            const Eigen::Vector2d &p,
                                            const Eigen::Vector2d &v,
                                            Eigen::Vector2d &gradp,
                                            double &gradt, double &grad_prev_t,
                                            double &costp) {
  if (i_dp <= 0 || i_dp >= cps_.cp_size * 2 / 3)
    return false;

  int size = swarm_trajs_->size();
  std::cout<<drone_id_<<"formation size"<<size<<std::endl;
  ...
  vector<Eigen::Vector2d> swarm_graph_pos(formation_size_),
      swarm_graph_vel(formation_size_);
  ...
  for (size_t id = 0; id < size; id++) {
    ...
    Eigen::Vector2d swarm_p, swarm_v;
    if (pt_time < traj_i_satrt_time + swarm_trajs_->at(id).duration) {
      ...
      double exceed_time =
          pt_time - (traj_i_satrt_time + swarm_trajs_->at(id).duration);
  ...
  //   vector<Eigen::Vector2d> swarm_grad;
...
bool PolyTrajOptimizer::obstacleGradCostP(const int i_dp,
  ...
  // use esdf
  double dist;
  grid_map_->evaluateEDT2d(p, dist);
  double dist_err = obs_clearance_ - dist;
  if (dist_err > 0) {
    ret = true;
    Eigen::Vector2d dist_grad;
    grid_map_->evaluateFirstGrad2d(p, dist_grad);
...
bool PolyTrajOptimizer::swarmGradCostP(const int i_dp, const double t,
  ...
  for (size_t id = 0; id < swarm_trajs_->size(); id++) {
    ...
    Eigen::Vector2d dist_vec = p - swarm_p;
    double ellip_dist2 =
...
void PolyTrajOptimizer::distanceSqrVarianceWithGradCost2p(
  ...
  for (int i = 0; i <= N; i++) {
    if (i != 0) {
      gdp.col(i) +=
          wei_sqrvar_ * (4.0 * (dsqrs(i - 1) - dsqrmean) / N * dps.col(i - 1));
    }
    if (i != N) {
      gdp.col(i) +=
          wei_sqrvar_ * (-4.0 * (dsqrs(i) - dsqrmean) / N * dps.col(i));
    }
  }
  return;
}
...
void PolyTrajOptimizer::astarWithMinTraj(const Eigen::MatrixXd &iniState,
  ...
  Eigen::Vector2d start_pos = iniState.col(0);
  Eigen::Vector2d end_pos = finState.col(0);

  /* astar search and get the simple path*/
  simple_path = a_star_->astarSearchAndGetSimplePath(grid_map_->getResolution(),
                                                     start_pos, end_pos);
  ...
  do {
    for (size_t i = 1; i <= piece_num; ++i) {
      time_vec(i - 1) =
          (i == 1) ? (simple_path[1] - start_pos).norm() / des_vel
...
bool PolyTrajOptimizer::getFormationPos(
    vector<Eigen::Vector2d> &swarm_graph_pos, Eigen::Vector2d pos) {
    ...
    for (size_t id = 0; id < swarm_trajs_->size(); id++) {
      ...
      if (pt_time < traj_i_satrt_time + swarm_trajs_->at(id).duration) {
        ...
        double exceed_time =
            pt_time - (traj_i_satrt_time + swarm_trajs_->at(id).duration);
        swarm_v =
            swarm_trajs_->at(id).traj.getVel(swarm_trajs_->at(id).duration);
        swarm_p =
            swarm_trajs_->at(id).traj.getPos(swarm_trajs_->at(id).duration) +
            exceed_time * swarm_v;
      }
...
void PolyTrajOptimizer::showFormationInformation(bool is_show,
                                                 Eigen::Vector2d pos) {
  if (!is_show)
    return;

  if (swarm_trajs_->size() < formation_size_ || drone_id_ != 0 ||
      !use_formation_) {
    return;
  } else {
    vector<Eigen::Vector2d> swarm_graph_pos(formation_size_);
    ...
    double similarity_error;
    swarm_graph_->calcFNorm2(similarity_error);
    cout << "[formation]: --------------- " << similarity_error
         << " --------------" << endl;
  }
}
...
void PolyTrajOptimizer::setEnvironment(const GridMap::Ptr &map) {
  ...
  a_star_->initGridMap(grid_map_, Eigen::Vector2i(800, 200));
}
