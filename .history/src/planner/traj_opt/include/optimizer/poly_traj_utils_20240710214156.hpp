...
public:
  ...
  // GaaiLam
  inline bool project_pt(const Eigen::Vector2d &pt, double &tt,
                         Eigen::Vector2d &pro_pt) {
    // 2*(p-p0)^T * \dot{p} = 0
  ...
  inline bool intersection_plane(const Eigen::Vector2d p,
                                 const Eigen::Vector2d v, double &tt,
...
public:
  ...
  inline bool project_pt(const Eigen::Vector2d &pt, int &ii, double &tt,
                         Eigen::Vector2d &pro_pt) {
    ...
    if (!find_project_pt) {
      // std::cout << "\033[32m" << "cannot project pt to traj" << "\033[0m" <<
      // std::endl; std::cout << "pt: " << pt.transpose() << std::endl;
      // assert(false);
    }
    return find_project_pt;
  }
  inline bool intersection_plane(const Eigen::Vector2d p,
                                 const Eigen::Vector2d v, int &ii, double &tt,
                                 Eigen::Vector2d &pt) {
    for (int i = 0; i < getPieceNum(); ++i) {
  ...
  getPoswithIdxRatio(double t, std::pair<int, double> &idx_ratio) const {
  ...
  inline double getLength() {
    ...
    for (int i = 0; i < N; i++) {
      Eigen::Vector2d vector =
          pieces[i].getPos(pieces[i].getDuration()) - pieces[i].getPos(0.0);
      length += vector.norm();
    }
...
private:
  template <typename EIGENVEC> inline void addGradJbyT(EIGENVEC &gdT) const {
    for (int i = 0; i < N; i++) {
  ...
  template <typename EIGENMAT> inline void addGradJbyC(EIGENMAT &gdC) const {
    for (int i = 0; i < N; i++) {
  ...
  inline void addTimeIntPenalty(const Eigen::VectorXi cons,
    ...
    for (int i = 0; i < N; i++) {
      ...
      for (int j = 0; j < innerLoop; j++) {
        ...
        K = cfgHs[idx].cols();
        for (int k = 0; k < K; k++) {
          outerNormal = cfgHs[idx].col(k).head<2>();
          violaPos = outerNormal.dot(pos - cfgHs[idx].col(k).tail<2>());
          if (violaPos > 0.0) {
            violaPosPenaD = violaPos * violaPos;
            violaPosPena = violaPosPenaD * violaPos;
            violaPosPenaD *= 3.0;
            gdC.block<6, 2>(i * 6, 0) += omg * step * ci(0) * violaPosPenaD *
...
public:
  inline void reset(const Eigen::Matrix<double, 2, 3> &headState,
  ...
  inline Trajectory getTraj(void) const {
    ...
    for (int i = 0; i < N; i++) {
      traj.emplace_back(
          T1(i), b.block<6, 2>(6 * i, 0).transpose().rowwise().reverse());
    }
    return traj;
  }
