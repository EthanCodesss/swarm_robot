...
# 读取 CSV 文件
data = pd.read_csv('/home/ethan/trajopt_dip/results/simu_results.csv')
...
plt.subplot(3, 2, 3)
trajectory_lengths = [data[f'Trajectory Length Iter {i}'] for i in range(num_iters)]
plt.plot(range(num_iters), trajectory_lengths, label='Trajectory Length', marker='o')
plt.xlabel('Iteration')
plt.ylabel('Trajectory Length (m)')
plt.title('Trajectory Length Over Iterations')
plt.legend()
