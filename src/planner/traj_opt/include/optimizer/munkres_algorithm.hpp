...
private:
  ...
  int STEP_0() {
    /* For each row r in C, subtract its smallest element from every element in
       r For each column c in C, subtract its smallest element from every
       element in c*/
    for (int row = 0; row < n_row_; row++) {
    ...
    /* For all zeros z_i in C, mark z_i with a star if there is no starred zero
  ...
  int STEP_1() {
    ...
    const bool covered = std::all_of(cover_col_.begin(), cover_col_.end(),
                                     [](bool v) { return v; });
  ...
  int STEP_2() {
    ...
    Step 3. Otherwise, cover this row and uncover the column containing the
    starred zero. Continue in this manner until there are no uncovered zeros
    left, Go to Step 2 Save the smallest uncovered value and Go to Step 4.*/

    if (uncovered_zero()) {
      for (int row = 0; row < n_row_; row++) {
        for (int col = 0; col < n_col_; col++) {
          if ((!cover_col_[col] && !cover_row_[row]) &&
              find_zero(row, col)) {        // find uncovered zero
            ...
            if (!row_starred(row)) {        // if no starred zero in row
              ...
              for (int column = 0; column < n_col_; column++) {
                if (star_matrix_[row]
                                [column]) { // uncover col containing star zero
  ...
  int STEP_3() {
    ...
    while (col_starred(Z0.second)) { // if column of Z0 has a starred zero Z1
      ...
      for (int col = 0; col < n_col_; col++) {
        if (prime_matrix_[Z1.first][col]) { // Replace Z0 with the primed zero
                                            // in the row of Z1
...
public:
  ...
  void run_munkres_algorithm(Eigen::MatrixXd C) {
    ...
    star_matrix_ = std::vector<std::vector<bool>>(
        n_row_, std::vector<bool>(n_col_, false));
    prime_matrix_ = std::vector<std::vector<bool>>(
        n_row_, std::vector<bool>(n_col_, false));
    emin_ = DBL_MAX;
    step_ = 0;
    ...
    if (show_debug_output) {
      std::cout << "[run_munkres_algorithm] "
                   "--------------------------------------------"
                << std::endl;
      dubug();
    }
