...
private:
  ...
  enum FORMATION_TYPE { NONE_FORMATION = 0, REGULAR_HEXAGON = 1 };
...
private:
  ...
  bool swarmGradCostP(const int i_dp, const double t, const Eigen::Vector2d &p,
                      const Eigen::Vector2d &v, Eigen::Vector2d &gradp,
  ...
  bool feasibilityGradCostV(const Eigen::Vector2d &v, Eigen::Vector2d &gradv,
                            double &costv);

  bool feasibilityGradCostA(const Eigen::Vector2d &a, Eigen::Vector2d &grada,
                            double &costa);

  void distanceSqrVarianceWithGradCost2p(const Eigen::MatrixXd &ps,
                                         Eigen::MatrixXd &gdp, double &var);

  void showFormationInformation(bool is_show, Eigen::Vector2d pos);

  bool checkCollision(void);

  bool getFormationPos(std::vector<Eigen::Vector2d> &swarm_graph_pos,
                       Eigen::Vector2d pos);

  void setDesiredFormation(int type) {
    std::vector<Eigen::Vector2d> swarm_des;
    switch (type) {
    ...
    case FORMATION_TYPE::REGULAR_HEXAGON: {
      ...
      Eigen::Vector2d v1(1.7321, -1);
      Eigen::Vector2d v2(0, -2);
      Eigen::Vector2d v3(-1.7321, -1);
      Eigen::Vector2d v4(-1.7321, 1);
      Eigen::Vector2d v5(0, 2);
      Eigen::Vector2d v6(1.7321, 1);
